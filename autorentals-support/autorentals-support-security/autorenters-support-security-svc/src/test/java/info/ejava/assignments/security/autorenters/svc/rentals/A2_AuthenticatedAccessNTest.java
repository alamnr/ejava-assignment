...
public class A2_AuthenticatedAccessNTest extends A1_AnonymousAccessNTest {
    ...
    void verify() {
        ...
        BDDAssumptions.given(env.getActiveProfiles()).as("missing required profile").contains("authenticated-access");
    ...
    void form_login_disabled(){
        BDDAssertions.then(filterChain.getFilters().stream()
                            ...
                            .findFirst().orElse(null))
                            .as("form authentication not disabled")
                            .isNull();
    }

    @Test
    void session_mgmt_disablrd(){
        // given
        URI url =  UriComponentsBuilder.fromUri(serverConfig.getBaseUrl()).path("path/whoAmI").build().toUri();
    ...
    class granted_authenticated_access_to {
        ...
        void renters_post() {
            //given
            RenterDTO validRenter = new RenterDTOFactory().make();
            //when
        ...
        void rentals_post(){
            ...
            TimePeriod timePeriod = new TimePeriod(LocalDate.now().now(),2);
            RentalDTO contract = authnHelper.makeProposal(auto, renter, timePeriod);

            // when
            ResponseEntity<RentalDTO> response = authnHelper.createContract(contract);
            // then
            BDDAssertions.then(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        }
