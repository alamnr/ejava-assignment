...
public class RenterRestClientNTest {
    ...
    public void init(@LocalServerPort int port ) {
        ...
        restClient.delete().uri(rentersUrl).retrieve().toEntity(Void.class) ;         
    }
    ...
    public void add_valid_renter_for_type(MediaType contentType, MediaType accept){
       ...
       // RenterDTO validRenter = renterDTOFactory.make();
        ...
        ResponseEntity<RenterDTO> response = restClient.post().uri(rentersUrl)
    ...
    void get_renter(){
        ...
        ResponseEntity<RenterDTO> response = restClient.post()
                                            .uri(rentersUrl).body(validRenter)
                                            .retrieve().toEntity(RenterDTO.class);
        ...
        ResponseEntity<RenterDTO> renterResponse  = restClient.get().uri(renterUrl).retrieve().toEntity(RenterDTO.class);
    ...
    void get_renters(String mediaTypeString){
        ...
        for (RenterDTO renter : renters.getRenters()) {
            ResponseEntity<RenterDTO> response = restClient.post().uri(rentersUrl).contentType(mediaType)
                                                .body(renter).retrieve().toEntity(RenterDTO.class);
            BDDAssertions.assertThat(response.getStatusCode().is2xxSuccessful()).isTrue();
    ...
    void add_valid_renter(MediaType contentType, MediaType accept) {
        ...
        // when / act 
        ResponseEntity<RenterDTO> response = restClient.post().uri(rentersUrl)
                                            .contentType(contentType).body(validRenter)
                                            .retrieve().toEntity(RenterDTO.class);
    ...
    private RenterDTO given_an_existing_renter(){
        ...
        ResponseEntity<RenterDTO> response =  restClient.post().uri(rentersUrl).accept(MediaType.APPLICATION_JSON)
    ...
    void update_existing_quote( ){
        ...
        ResponseEntity<RenterDTO> getUpdatedQuote = restClient.get().uri(getUri).retrieve().toEntity(RenterDTO.class);
    ...
    void get_renter_1() {
        ...
        RenterDTO existingRenter = renterDTOFactory.make();
        ResponseEntity<RenterDTO> response = restClient.post().uri(rentersUrl)
                                            .body(existingRenter).retrieve().toEntity(RenterDTO.class);
        ...
        ResponseEntity<RenterDTO> getRenter =  restClient.get().uri(UriComponentsBuilder.fromUri(baseUrl)
    ...
    protected List<RenterDTO> given_many_renters(int count) {
        ...
        for (RenterDTO renterDTO : renterDTOFactory.listBuilder().renters(count,count)) {
                ResponseEntity<RenterDTO> response = restClient.post().uri(rentersUrl).body(renterDTO).retrieve().toEntity(RenterDTO.class);
    ...
    void remove_renter(){
        ...
        BDDAssertions.assertThat(restClient.get().uri(UriComponentsBuilder.fromUri(baseUrl).path(RentersAPI.RENTER_PATH).build(requestId))
                                          .retrieve().toEntity(RenterDTO.class).getStatusCode()).isEqualTo(HttpStatus.OK);
        ...
        RestClientResponseException ex = BDDAssertions.catchThrowableOfType(
                                            ...
                                            .path(RentersAPI.RENTER_PATH).build(requestId)).retrieve().toEntity(RenterDTO.class),
    ...
    void remove_all_renters() {
        ...
        ResponseEntity<Void> resp = restClient.delete().uri(rentersUrl).retrieve().toEntity(Void.class);
                                           
        // then
        BDDAssertions.then(resp.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
        for (RenterDTO renterDTO : renters) {
               RestClientResponseException ex = BDDAssertions.catchThrowableOfType(
                        ()-> restClient.get().uri(UriComponentsBuilder.fromUri(baseUrl).path(RentersAPI.RENTER_PATH)
                        .build(renterDTO.getId())).retrieve().toEntity(RenterDTO.class),
                         RestClientResponseException.class) ;
                BDDAssertions.assertThat(ex.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        }
    ...
    void remove_unknown_renter() {
        ...
        ResponseEntity<Void> resp = restClient.delete().uri(UriComponentsBuilder.fromUri(baseUrl).path(RentersAPI.RENTER_PATH).build(requestId))
                                        .retrieve().toEntity(Void.class);
    ...
    void get_random_renter_no_renters(){
        // given
        BDDAssertions.assertThat( restClient.delete().uri(rentersUrl)
                        .retrieve().toEntity(Void.class).getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
               
        // then
        RestClientResponseException ex = BDDAssertions.catchThrowableOfType(
            () -> restClient.get().uri(UriComponentsBuilder.fromUri(baseUrl).path(RentersAPI.RENTER_PATH).build("renter-1"))
                            .retrieve().toEntity(RenterDTO.class),
                             RestClientResponseException.class);
    ...
    void get_unknown_renter(){
        ...
        RestClientResponseException ex = BDDAssertions.catchThrowableOfType(
            () -> restClient.get().uri(UriComponentsBuilder.fromUri(baseUrl).path(RentersAPI.RENTER_PATH).build(unknownId))
                                    .retrieve().toEntity(RenterDTO.class),RestClientResponseException.class);
        // then
        BDDAssertions.then(ex.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        MessageDTO errMsg = getErrorResponse(ex);
    ...
    void update_unknown_renter() {
        ...
        RestClientResponseException ex =  BDDAssertions.catchThrowableOfType(
            ()->  restClient.put().uri(UriComponentsBuilder.fromUri(baseUrl).path(RentersAPI.RENTER_PATH).build(unknownId))
                            .body(updateRenter).retrieve().toEntity(Void.class), RestClientResponseException.class);
    ...
    void update_known_renter_with_bad_renter() {
        ...
        RestClientResponseException ex = BDDAssertions.catchThrowableOfType(
         () -> restClient.put().uri(UriComponentsBuilder.fromUri(baseUrl).path(RentersAPI.RENTER_PATH).build(unknownId))
                        .body(badRenterMissingText).retrieve().toEntity(Void.class),
                         RestClientResponseException.class);
        // then
        BDDAssertions.then(ex.getStatusCode()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY);
    ...
    void add_bad_renter_rejected() {
        ...
        () ->  restClient.post().uri(rentersUrl).contentType(contentType)
                                .accept(MediaType.valueOf(MediaType.APPLICATION_JSON_VALUE)).body(badRenterMissingText)
                                .retrieve().toEntity(RenterDTO.class), RestClientResponseException.class);
        // then
        BDDAssertions.then(ex.getStatusCode()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY);
    ...
    void get_empty_renters(){
        ...
        ResponseEntity<RenterListDTO> response = restClient.get().uri(url).retrieve().toEntity(RenterListDTO.class);
