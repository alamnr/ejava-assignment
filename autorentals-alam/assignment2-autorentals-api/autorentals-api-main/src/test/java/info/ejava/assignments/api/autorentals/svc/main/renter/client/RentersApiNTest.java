...
public class RentersApiNTest {
    ...
    class query {
        ...
        void can_get_renters() {
            ...
            //then
            then(response.getStatusCode()).isEqualTo(HttpStatus.OK);
            List<RenterDTO> foundRenters = response.getBody().getRenters();
        ...
        void can_get_renters_paged_list() {
            ...
            do {
                ...
                then(renters.getRenters()).isNotNull();
                then(renters.getRenters().size()).isLessThanOrEqualTo(pageSize);
                returnedRenters.addAll(renters.getRenters());
                if (renters.getRenters().isEmpty()) {
                    break;
                }
                pageNum += 1;
            } while (true);
