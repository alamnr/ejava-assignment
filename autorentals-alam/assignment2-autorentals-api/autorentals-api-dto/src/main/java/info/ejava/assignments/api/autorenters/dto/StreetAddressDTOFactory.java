...
public class StreetAddressDTOFactory {
    ...
    public final StreetAddressDTO make(Consumer<StreetAddressDTO>... visitors) {
        ...
        final StreetAddressDTO result = StreetAddressDTO.builder().street(address.streetAddress())
                                            .city(address.city()).state(address.stateAbbr()).zip(address.zipCode())
                                            .build();
        Stream.of(visitors).forEach(v->v.accept(result));
        return result;
    }

    public StreetAddressesDTOFactory listBuilder(){
        return new StreetAddressesDTOFactory();
    }
